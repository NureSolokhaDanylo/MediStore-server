        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    // get batches that are not yet expired (DB-side) to avoid processing already expired ones
                    var candidates = await uow.Batches.GetBatchesApproachingExpirationAsync(DateTime.UtcNow);
                    ...
                    foreach (var batch in candidates)
                    {
                        try
                        {
                            ...
                            if (thresholdDate < batch.ExpireDate) continue;
